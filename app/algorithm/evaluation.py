"""
Evaluation / fitness function for timetable individuals.

Ported from GA-GWO15 notebook — uses numpy matrix representation for
fast vectorised constraint checking.
"""
...
class Evaluator:
    """Evaluates timetable fitness based on hard and soft constraints.

    All evaluation functions work on the *matrix* representation:
    ...
    # ── Individual constraint checks (return raw violation count) ─────

    def guru_bentrok(self, guru_matrix: np.ndarray) -> int:
        """Check if any teacher teaches multiple classes in the same time slot."""
        pelanggaran = 0
    ...
    def durasi_guru(self, guru_matrix: np.ndarray) -> int:
        """Check if teacher total hours exceed maximum allowed."""
        pelanggaran = 0
    ...
    def distribusi_mapel(self, mapel_matrix: np.ndarray) -> int:
        """Check subject distribution across days and block structure."""
        pelanggaran = 0
        ...
        for kelas in range(jumlah_kelas):
            ...
            for mapel, slots in mapel_slot.items():
                ...
                # Only check subjects with >1 block
    ...
    def mapel_siang(self, mapel_matrix: np.ndarray) -> int:
        """PE (mapel ID 8) should not be in afternoon slots."""
        TARGET_MAPEL = 8
        pelanggaran = 0
    ...
    def waktu_mgmp(self, mapel_matrix: np.ndarray) -> int:
        """Check MGMP (teacher meeting) day constraints."""
        pelanggaran = 0
        jumlah_kelas = self._d.jumlah_kelas
    ...
    def cek_wali_kelas(self, guru_matrix: np.ndarray) -> int:
        """Check if homeroom teachers teach in their own class."""
        pelanggaran = 0
    ...
    def konsistensi_guru_mapel(
        ...
        """Check if each subject in a class is taught by exactly one teacher."""
        pelanggaran = 0
        jumlah_kelas = self._d.jumlah_kelas
    ...
    def evaluasi_individu(
        ...
        """Evaluate a single individual and return violation counts per constraint."""
        return {
    ...
    def hitung_fitness(self, hasil_evaluasi: dict[str, int]) -> float:
        """Calculate weighted fitness from violation counts."""
    ...
    def evaluate(
        ...
        """
        Full evaluation of one individual.

        Returns:
            (fitness_score, evaluation_detail_dict)
        """
    ...
    def evaluasi_populasi(
        ...
        Returns a list of dicts with keys: fitness, evaluasi, index.
        """
        hasil_populasi = []
    ...
    def _hitung_blok(slots: list[int]) -> list[int]:
        """Count contiguous block lengths from a list of slot indices."""
        slots = sorted(slots)
        blok: list[int] = []
        panjang = 1
