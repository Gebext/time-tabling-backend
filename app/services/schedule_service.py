"""
Service layer for Schedule generation using the hybrid GA-GWO algorithm.

Ported from GA-GWO15 notebook — runs GA with periodic GWO optimisation
...
# Default GWO parameters (can be overridden via ScheduleRequest)
...
class ScheduleService:
    ...
    - Start a generation job (runs in background thread)
    ...
    # ── Internal — Hybrid GA-GWO Loop ────────────────────

    def _run_algorithm(self, params: ScheduleRequest) -> None:
        """
        Actual algorithm execution (runs in worker thread).

        Matches the notebook's runGA() which interleaves GA generations
        with periodic GWO optimisation phases.
        """
        try:
            ...
            for gen in range(max_iter):
                ...
                # ═══════════════════════════════════════
                # GWO PHASE (every GWO_INTERVAL generations)
                # ═══════════════════════════════════════
    ...
    def _format_result(
        ...
        """Format the best individual into a structured result."""
